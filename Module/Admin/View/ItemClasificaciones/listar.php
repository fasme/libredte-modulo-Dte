...
foreach ($Objs as &$obj) {
    ...
    if ($deleteRecord) {
        $actions .= ' <a href="'.$_base.$module_url.$controller.'/eliminar/'.$obj->codigo.$listarFilterUrl.'" title="Eliminar" onclick="return eliminar(\''.$model.'\', \''.implode(', ', $pkValues).'\')"><span class="fa fa-remove btn btn-default"></span></a>';
    }
    $row[] = $actions;
    $data[] = $row;
}
