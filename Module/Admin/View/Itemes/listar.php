...
<h1>Listado de productos y/o servicios</h1>
<p><?=$comment?></p>
...
foreach ($Objs as &$obj) {
    ...
    if ($deleteRecord) {
        $actions .= ' <a href="'.$_base.$module_url.$controller.'/eliminar/'.$obj->codigo.'/'.$obj->codigo_tipo.$listarFilterUrl.'" title="Eliminar" onclick="return eliminar(\''.$model.'\', \''.implode(', ', $pkValues).'\')"><span class="fa fa-remove btn btn-default"></span></a>';
    }
    $row[] = $actions;
    $data[] = $row;
}
